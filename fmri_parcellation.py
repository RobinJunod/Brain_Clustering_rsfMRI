...
def watershed_by_flooding(gradient_magnitude_map: np.ndarray, 
    ...
    """
    Perform watershed algorithm on a 3D gradient magnitude map starting from the seeds.

    Args:
        gradient_magnitude_map (np.ndarray): The 3D gradient magnitude map.
        seeds_maps (np.ndarray): The 3D seeds map, where non-zero values indicate seed points.

    Returns:
        np.ndarray: A 3D array of labeled regions after applying the watershed algorithm.
    """
    import heapq
